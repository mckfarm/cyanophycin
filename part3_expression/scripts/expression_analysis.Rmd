...
wang <- left_join(annotations, kallisto, by = "locus_tag") %>%
  ...
  mutate(bin = ifelse(bin == "Acc_IA", "IA", ifelse(bin == "Acc_IC", "IC", "IF"))) %>%
...
guide_wang_tpm <- guides(fill = guide_legend(title = "Gene", 
                                             override.aes = list(color = met.brewer("Derain", 4),
                                                    shape = c(15, 15, 15, 15), 
                                                    size = c(3, 3, 3, 3)),
                                             nrow = 1))
...
wang_tpm_box <-
  wang_tpm_filt %>% filter(product_short %in% c("cphA", "glgC", "phaC", "ppk")) %>%
...
wang_tpm_filt %>% filter(product_short %in% c("cphA", "glgC", "phaC", "ppk")) %>%
  ...
  scale_fill_manual(values = met.brewer("Derain", 4), guide = guide_wang_tpm) +
  theme_bw() + 
...
(wang_tpm_grid + wang_tpm_box + 
    plot_layout(widths = c(1, 0.4), guides = "collect") & theme(legend.position = "bottom")) + 
...
ggsave(file.path(outpath_figs, "expression_wang_cpha.tiff"), dpi = 300, width = 8, height = 6, units = "in")
