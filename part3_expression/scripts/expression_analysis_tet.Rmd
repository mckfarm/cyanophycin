...
guide_cols <- c("goldenrod1", "seagreen3", "royalblue4")

guide_tpm <- guides(fill = guide_legend(title = "Gene", 
                                             override.aes = list(
                                                    color = guide_cols,
                                                    shape = c(15, 15, 15), 
                                                    size = c(3, 3, 3)),
...
tet_box <- 
tet_filt %>% filter(product_short %in% c("cphA", "glgC", "phaC", "ppk")) %>%
...
ggsave(file.path(outpath_figs, "expression_tet.tiff"), dpi = 300, width = 8, height = 4.5, units = "in")
...
tet_tpm %>% filter(product == "Cyanophycin synthetase") %>%
  ...
  scale_fill_manual(values = met.brewer("OKeeffe2", 2), name = "Gene") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
  labs(x = "Reactor Phase", y = "Transcripts per million reads") +
  theme_bw() +
...
ggsave(file.path(outpath_figs, "expression_tet_cpha.tiff"), dpi = 300, width = 6, height = 4, units = "in")
